...
pool.getConnection(function(err, connection) {
    ...
    VaccineR.prototype.save = function save(admin,callback) {
        pool.getConnection(function (err, connection) {
            ...
            var insertvaccine_Sql = "INSERT INTO vaccine(username,petname,vaccinename,petage,status) VALUES(?,?,?,?,?)";

            connection.query(insertvaccine_Sql, [admin.username,admin.petname,admin.vaccinename,admin.petage,admin.status], function (err, result) {

                connection.release();
                if (err) {
                    console.log("insertvaccine_Sql Error: " + err.message);
                    return;
                }
                callback(err, result);
            });
    ...
    VaccineR.getvaccine= function getvaccine(username,petname,callback) {
        
       pool.getConnection(function(err, connection) { 
            var getvaccine_sql =  "SELECT * FROM vaccine WHERE username = ? && petname=? order by vaccineid desc";
            var getvaccine_sql1 = "SELECT * FROM vaccine order by vaccineid desc";
            if(username!='undefined'&&petname!='undefined'){
                connection.query(getvaccine_sql , [username,petname], function (err, result) {  
                    if (err) {
                        console.log("getvaccine_sql Error: " + err.message);
                        return;
                    }
                    callback(err, result);
                ...
                connection.query(getvaccine_sql1, function (err, result) {  
                    if (err) {
                        console.log("getvaccine_sql1 Error: " + err.message);
                        return;
                    }
                    callback(err, result);

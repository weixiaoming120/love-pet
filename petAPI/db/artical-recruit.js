...
pool.getConnection(function(err, connection) {
    ...
    //根据用户名得到用户信息
    ArticalR.getartical = function getartical(articalid, callback) {
        
        pool.getConnection(function (err, connection) {
            var get_Sql = "SELECT * FROM artical WHERE articalid = ?";
            var get_Sql1 = "SELECT * FROM artical";

            //如果username存在，则返回相关用户信息
            if(articalid != undefined){
                connection.query(get_Sql, [articalid], function (err, result) {  
                    if (err) {
